...
const file = path.resolve(__dirname, "../public/data/senators.json");

const raw = fs.readFileSync(file, "utf8");
const parsed = JSON.parse(raw);

if (!Array.isArray(parsed) || parsed.length === 0) {
  throw new Error("senators.json is empty or not an array");
}
...
console.log(`senators.json OK (${parsed.length})`);
