...
const DATA_PATH = path.join(WEB_ROOT, "public", "data", "representatives.json");
const FIX_TARGETS_PATH = path.join(WEB_ROOT, "public", "data", "representatives-image-fix-targets.json");
const SEARCH_TARGETS_PATH = path.join(WEB_ROOT, "public", "data", "representatives-image-search-targets.json");
const MISSING_PATH = path.join(WEB_ROOT, "public", "data", "missing-images.json");

const FORCE_FIX_NAMES = new Set([
  "浅田眞澄美",
  "石田真敏",
  "伊藤聡",
  "岩崎比菜",
...
function readTargets(members) {
  const mode = String(process.env.REP_IMAGE_TARGET_MODE || "missing").trim().toLowerCase();
  if (mode === "fix") {
    return {
      mode: "fix",
      names: [...new Set([...FORCE_FIX_NAMES].map(cleanName).filter(Boolean))]
    };
  }
...
function buildTargetIndex(members, targetNames) {
  ...
  return { targetMembers, byExact, byKana };
}
...
function candidateImageUrls(attrs = {}, base = "") {
  const raws = [attrs.src, attrs["data-src"], attrs["data-original"], attrs["data-lazy-src"], attrs["data-srcset"], attrs.srcset];
  const out = [];
...
function collectDirectAltMatches(html = "", pageUrl = "", indexes) {
  ...
  $("img[alt]").each((_, el) => {
    ...
    if (!best || shouldSkipImage(best.url, attrs.alt || "")) return;
...
async function buildNhkPages() {
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      pages.push({ url, html });
      for (const next of collectNhkLinks(html, url)) {
...
async function main() {
  if (!fs.existsSync(DATA_PATH)) throw new Error(`representatives.json not found: ${DATA_PATH}`);
  ...
  console.log(`rep-image mode=${targetInfo.mode} total=${members.length} targets=${indexes.targetMembers.length} target-file=${FIX_TARGETS_PATH}`);

  if (targetInfo.mode === "fix") {
    const fixPayload = indexes.targetMembers.map((t) => ({ name: normalizeSpace(t.member.name) }));
    fs.writeFileSync(FIX_TARGETS_PATH, JSON.stringify(fixPayload, null, 2));
  }
  ...
  for (const page of nhkPages) {
    ...
    console.log(`rep-image nhk-page url=${page.url} direct=${direct.length}`);
    for (const item of direct) {
      const prev = bestByName.get(item.memberName);
      if (!prev || item.score > prev.score) bestByName.set(item.memberName, item);
    }
  }
  ...
  const stillMissing = [];
  for (const target of indexes.targetMembers) {
    if (!bestByName.has(target.memberName)) {
      stillMissing.push(normalizeSpace(target.member.name));
