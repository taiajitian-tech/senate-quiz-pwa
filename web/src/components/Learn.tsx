...
function findPersonById(items: Person[], id: number) {
...
export default function Learn(props: Props) {
  ...
  const [mistakeCursor, setMistakeCursor] = useState(0);
  const [mistakeResult, setMistakeResult] = useState<ReviewResult>({ remembered: 0, hazy: 0, notRemembered: 0 });
  ...
  useEffect(() => {
    ...
    setMistakeCursor(0);
    setMistakeResult({ remembered: 0, hazy: 0, notRemembered: 0 });
    setRevealed(false);
  ...
  const current = useMemo(() => {
    if (mistakeMode) {
      const mistakeId = mistakeIds[mistakeCursor];
      return typeof mistakeId === "number" ? findPersonById(items, mistakeId) : null;
    }
    if (sessionDone || askedIds.length >= SESSION_SIZE) return null;
    return pickNext(items, progress, Date.now(), props.mode, askedIdSet);
  }, [items, progress, props.mode, askedIdSet, askedIds.length, sessionDone, mistakeMode, mistakeIds, mistakeCursor]);

  useEffect(() => {
    if (loading || mistakeMode) return;
    if (!sessionDone && (askedIds.length >= SESSION_SIZE || (askedIds.length > 0 && !current))) {
      setSessionDone(true);
      setRevealed(false);
    }
  }, [askedIds.length, current, loading, sessionDone, mistakeMode]);


  const mistakeTotal = mistakeIds.length;
  const mistakeRemaining = mistakeMode ? Math.max(mistakeTotal - mistakeCursor, 0) : mistakeTotal;
  const hasMistakeReview = mistakeIds.length > 0;
  ...
  const onGrade = (grade: Grade) => {
    ...
    if (mistakeMode) {
      setMistakeResult((prevResult) => ({
        remembered: prevResult.remembered + (grade === "good" || grade === "strong" ? 1 : 0),
        hazy: prevResult.hazy + (grade === "hard" ? 1 : 0),
        notRemembered: prevResult.notRemembered + (grade === "again" ? 1 : 0),
      }));
      const nextCursor = mistakeCursor + 1;
      if (nextCursor >= mistakeIds.length) {
        setMistakeMode(false);
        setMistakeCursor(0);
      } else {
        setMistakeCursor(nextCursor);
      }
      setRevealed(false);
      return;
    }
    ...
    if (grade === "again" || grade === "hard") {
      setMistakeIds((prevIds) => (prevIds.includes(current.id) ? prevIds : [...prevIds, current.id]));
    }
          ...
          {loading ? <div style={styles.center}>読み込み中</div> : sessionDone ? (
            <div style={styles.doneWrap}>
              ...
              <div style={styles.doneMeta}>
                ...
                <div>今回のまとめ復習候補：{mistakeTotal}人</div>
              </div>
              {hasMistakeReview ? (
                <div style={styles.reviewCallout}>
                  <div style={styles.reviewCalloutTitle}>間違えた議員だけ再確認</div>
                  <div style={styles.reviewCalloutText}>今回のうろ覚え・覚えていない議員を、まとめて復習できます。</div>
                  <button type="button" style={styles.primaryBtn} onClick={startMistakeReview}>まとめ復習を開始</button>
                </div>
              ) : null}
              {mistakeResult.remembered + mistakeResult.hazy + mistakeResult.notRemembered > 0 ? (
                <div style={styles.reviewResultBox}>
                  <div style={styles.reviewCalloutTitle}>まとめ復習の結果</div>
                  <div style={styles.reviewResultGrid}>
                    <div style={styles.reviewResultItem}>覚えていた {mistakeResult.remembered}</div>
                    <div style={styles.reviewResultItem}>うろ覚え {mistakeResult.hazy}</div>
                    <div style={styles.reviewResultItem}>覚えていない {mistakeResult.notRemembered}</div>
                  </div>
                </div>
              ) : null}
            ...
            <div style={compactLayout ? styles.quizLayoutCompact : styles.quizLayout}>
              {mistakeMode ? <div style={styles.reviewBanner}>まとめ復習 残り {mistakeRemaining} 人</div> : null}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  quizLayout: { display: "grid", gridAutoRows: "auto", gridTemplateRows: "auto minmax(0, 1fr) auto auto", gap: 8, width: "100%", minHeight: 0 },
  quizLayoutCompact: { display: "grid", gridAutoRows: "auto", gridTemplateRows: "auto minmax(0, 1fr) auto auto", gap: 6, width: "100%", minHeight: 0 },
  ...
  reviewBanner: { padding: "6px 9px", borderRadius: 10, background: "#eef6ff", border: "1px solid #c8ddff", color: "#0958b3", fontSize: 12, fontWeight: 700, textAlign: "center" },
  ...
  reviewCallout: { display: "grid", gap: 8, padding: 10, borderRadius: 12, background: "#f8fafc", border: "1px solid #d7e3f4" },
  reviewCalloutTitle: { fontSize: 14, fontWeight: 800, color: "#123b6d" },
  reviewCalloutText: { fontSize: 12, color: "#445", lineHeight: 1.45 },
  reviewResultBox: { display: "grid", gap: 8, padding: 10, borderRadius: 12, background: "#faf7ff", border: "1px solid #e4d8ff" },
  reviewResultGrid: { display: "grid", gap: 6 },
  reviewResultItem: { fontSize: 12, color: "#444" },
