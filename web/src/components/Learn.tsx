...
function pickNext(
  ...
  const masteredReview = [...masteredLocked].sort((a, b) => {
    const aDue = progress[a.id]?.due ?? Number.MAX_SAFE_INTEGER;
    const bDue = progress[b.id]?.due ?? Number.MAX_SAFE_INTEGER;
    return aDue - bDue;
  });
  ...
  const shouldReintroduceMastered =
    masteredReview.length > 0 &&
    fresh.length === 0 &&
    riskLeech.length === 0 &&
    Math.random() < 0.05;
  ...
  if (shouldReintroduceMastered) return masteredReview[0];
  if (fresh.length > 0) return fresh[Math.floor(Math.random() * fresh.length)];
  return riskLeech[0] ?? riskDue[0] ?? riskUpcoming[0] ?? masteredReview[0] ?? null;
}
