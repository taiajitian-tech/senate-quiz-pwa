    ...
    <div style={styles.wrap}>
      <div style={styles.card}>
        <div style={styles.titleRow}>
          <div>
            ...
            <div style={styles.titleSub}>まず流れで覚えて、その後に思い出す回数を増やす学習アプリ</div>
          </div>
        ...
        <div style={styles.targetRow}>
          ...
          <div style={styles.targetHint}>迷ったら「最初に」→「自動再生」→「学習」の順です</div>
        </div>

        <div style={styles.startBox}>
          <div style={styles.startHeader}>
            <div style={styles.startTitle}>まず使う場所</div>
            <div style={styles.startSub}>最初の2つだけ見れば始められます</div>
          </div>
          <div style={styles.startGrid}>
            <MenuButton
              ...
              label="最初に"
              sub="使い方と覚え方を先に見る"
              onClick={props.onOpenFirstGuide}
            />
            <MenuButton
              variant="primary"
              label="学習"
              sub="基本モード。顔を見て名前を答える"
        ...
        <div style={styles.menuSection}>
          <div style={styles.sectionTitle}>学習メニュー</div>
          <div style={styles.menu}>
            ...
            <MenuButton label="復習" sub="忘れかけだけを出して固める" onClick={props.onStartReview} />
            <MenuButton label="一覧" sub="全体を見渡して確認する" onClick={props.onOpenList} />
            <MenuButton label="成績確認" sub="覚えた人数と進み具合を見る" onClick={props.onOpenStats} />
        ...
        <div style={styles.guideBox}>
          ...
          <div style={styles.stepGrid}>
            <div style={styles.stepCard}><div style={styles.stepNum}>1</div><div><b>最初に</b><div style={styles.stepText}>使い方を見て、迷わず始める</div></div></div>
            <div style={styles.stepCard}><div style={styles.stepNum}>2</div><div><b>自動再生</b><div style={styles.stepText}>まず多くの顔を見る（おすすめ: 顔2秒・名前2秒）</div></div></div>
            <div style={styles.stepCard}><div style={styles.stepNum}>3</div><div><b>学習</b><div style={styles.stepText}>顔を見て名前を思い出す</div></div></div>
            <div style={styles.stepCard}><div style={styles.stepNum}>4</div><div><b>逆学習・復習</b><div style={styles.stepText}>名前から顔も引けるようにして定着させる</div></div></div>
          </div>
          <div style={styles.memoryBox}>
            <div style={styles.memoryTitle}>迷ったときの基準</div>
            <div style={styles.memoryText}>3秒以内に出なければ考え込みすぎず答えを見た方が進みます。最初は広く見て、その後に思い出す回数を増やす流れが定着しやすいです。</div>
          </div>
        </div>
      </div>
      ...
      <HelpModal open={helpOpen} onClose={() => setHelpOpen(false)} title="ヘルプ（アプリ）">
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div><b>最初にやること</b></div>
          <div>最初は「最初に」で流れを見てから、自動再生で顔を広く見てください。その後に学習へ入ると、初回から覚えやすくなります。</div>
          <div><b>迷ったときの基準</b></div>
          <div>3秒以内に出なければ、考え込まずに答えを見た方が効率が上がります。</div>
          <div><b>復習の役割</b></div>
          <div>復習は、すでに見た議員の中から今ちょうど忘れかけの議員だけを出します。定着に大事な部分です。</div>
        </div>
      </HelpModal>
    </div>
  );
...
const styles: Record<string, React.CSSProperties> = {
  ...
  titleSub: { fontSize: 14, color: "#555", marginTop: 4, lineHeight: 1.6 },
  ...
  targetRow: { width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" },
  targetLabel: { fontSize: 14, color: "#444" },
  targetHint: { fontSize: 13, color: "#6b7280" },
  ...
  startSub: { fontSize: 13, color: "#666" },
  startGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 },
  ...
  sectionTitle: { fontSize: 18, fontWeight: 800, alignSelf: "flex-start" },
  ...
  stepText: { fontSize: 13, color: "#555", marginTop: 4, lineHeight: 1.5 },
  ...
  guideCardBtn: { ...commonBtn, border: "1px solid #eab308", background: "#fff9db", textAlign: "left" },
